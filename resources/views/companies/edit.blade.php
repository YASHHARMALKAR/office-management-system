...
<form method="POST" action="/companies/{{$company->id}}" class="bg-white p-6 rounded shadow w-96">
@csrf
@method('PUT')

<h2 class="text-xl font-bold mb-4 text-center">Edit Company</h2>

<input name="name" value="{{$company->name}}"
class="w-full border p-2 mb-3 rounded">

<input name="location" value="{{$company->location}}"
class="w-full border p-2 mb-3 rounded">
